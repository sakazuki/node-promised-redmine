...
var proto = {
	...
	, setRejectUnauthorized: function(c){ this.rejectUnauthorized = c; return this}
	, getRejectUnauthorized: function(){ return this.rejectUnauthorized}
	...
	, request: function(method, path, params){
		...
		req = (self.protocol==='https' ? https : http).request(options, function(res){
			...
			res.on('end', function() {
				if( !~([200,201]).indexOf(res.statusCode) ){
					...
					var data = {message: 'STATUSCODE_REJECTION ', code: res.statusCode, body: JSON.parse(body || res.statusCode)};
					d.reject(data);
					return;
				}
				try {
		...
		if ((method === 'POST') && params.content){
			req.setHeader('Content-Type', 'application/octet-stream');
			req.setHeader('Content-Length', params.content.byteLength)
	...
	*/
	...
	, deleteIssue: function(id, params) { return this.del('issues/' + id, params); }
	, upload: function(attachment) {
		return uploads([attachment]);
	}
	, uploads: function(attachments) {
		var d1 = D();
		var self = this;
		D.all(
			attachments.map(
				function(attachment){
					var d2 = D();
					self.post("uploads", {"content": attachment.content}).then(function (data){
						data.upload.filename = attachment.fileName;
						data.upload.content_type = attachment.contentType;
						d2.resolve(data.upload);
					}, function(err){
						console.log(err);
						d2.reject(err);
					})
	...
	///----- ISSUE STATUSES METHODS -----///
	,getIssueStatuses: function(params){ return this.get('issue_statuses', params); }
